...
//根据alpha   beta 推断 gamma （theta的后验），对应LDA论文中的Figure.6
double lda_inference(document* doc, lda_model* model, double* var_gamma, double** phi)
{
    ...
    {
        var_gamma[k] = model->alpha + (doc->total/((double) model->num_topics));   //1.给gamma一个初始值
        digamma_gam[k] = digamma(var_gamma[k]);
        for (n = 0; n < doc->length; n++)
            phi[n][k] = 1.0/model->num_topics;   //2.给phi一个初始值
    }
    var_iter = 0;
